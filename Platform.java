...
public class Platform implements Serializable {
    ...
    // username -> User (stored in lower-case key)
    ...
    public void loadData() {
        ...
        if (pd != null) {
            this.users = pd.users != null ? pd.users : new HashMap<>();
            this.recipes = pd.recipes != null ? pd.recipes : new HashMap<>();
    ...
    /* =====================  RECIPE OPERATIONS (CORE)  ===================== */

    public Recipe addRecipe(String authorUsername,
                            String title,
                            String description,
    ...
    public List<Recipe> searchByIngredient(String ingredient) {
        ...
        return recipes.values().stream()
                .filter(r -> r.getIngredients().stream()
                        .anyMatch(ing -> ing.toLowerCase().contains(ql)))
                .sorted(Comparator.comparing(Recipe::getCreatedAt).reversed())
                .collect(Collectors.toList());
    }
    ...
    public List<Recipe> searchByTag(String tag) {
        ...
        return recipes.values().stream()
                .filter(r -> r.getTags().stream()
                        .anyMatch(t -> t.toLowerCase().equals(ql)))
                .sorted(Comparator.comparing(Recipe::getCreatedAt).reversed())
                .collect(Collectors.toList());
    }
    ...
    /** Generic text search used by /search servlet. */
    public List<Recipe> searchRecipes(String query) {
        ...
        // title OR description OR tag OR ingredient match
        return recipes.values().stream()
                .filter(r ->
                        (r.getTitle() != null && r.getTitle().toLowerCase().contains(q)) ||
                        (r.getDescription() != null && r.getDescription().toLowerCase().contains(q)) ||
                        r.getTags().stream().anyMatch(t -> t.toLowerCase().contains(q)) ||
                        r.getIngredients().stream().anyMatch(ing -> ing.toLowerCase().contains(q)))
                .sorted(Comparator.comparing(Recipe::getCreatedAt).reversed())
                .collect(Collectors.toList());
    }

    /** Recipes authored by a given user, for /user/menu servlet. */
    public List<Recipe> getRecipesByUser(User user) {
        ...
        return recipes.values().stream()
                .filter(r -> r.getAuthorUsername().equalsIgnoreCase(uname))
                .sorted(Comparator.comparing(Recipe::getCreatedAt).reversed())
                .collect(Collectors.toList());
    }
    ...
    // === existing interactive console helpers (unchanged) ===
    ...
    private void printRecipeSummaries(List<Recipe> list) {
        ...
        for (Recipe r : list) {
            System.out.printf("[%d] %s (by %s) - likes: %d comments: %d\n",
                    r.getId(), r.getTitle(), r.getAuthorUsername(),
                    r.getLikes().size(), r.getComments().size());
        }
    }
    ...
    private void editRecipeInteractive(Scanner sc, int id, User user) {
        ...
        List<String> newTags = tagLine.isEmpty()
                ? r.getTags()
    ...
    private void followUserInteractive(Scanner sc, User user) {
        ...
        System.out.println(added ? "Now following " + target.getUsername()
                : "You already follow " + target.getUsername());
    }
